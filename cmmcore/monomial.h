...
inline void getVectorTensor2DComponent(Tensor2D<T>& arr,  int component,Matrix& result) {
            ...
            for (int j1 = 0; j1 < m1; ++j1) {
                for (int i2 = 0; i2 < n2; ++i2) {
                    for (int j2 = 0; j2 < m2; ++j2) {
                        int i = i1 + i2;
                        int j = j1 + j2;
                        result[i][j][0] += (a_coeffs[i1][j1][1] * b_coeffs[i2][j2][2] - a_coeffs[i1][j1][2] * b_coeffs[i2][j2][1]);
                        result[i][j][1] += (a_coeffs[i1][j1][2] * b_coeffs[i2][j2][0] - a_coeffs[i1][j1][0] * b_coeffs[i2][j2][2]);
                        result[i][j][2] += (a_coeffs[i1][j1][0] * b_coeffs[i2][j2][1] - a_coeffs[i1][j1][1] * b_coeffs[i2][j2][0]);
                    }
                }
            }
        }
    ...
    Monomial2D(int _n, int _m): n(_n), m(_m), coefficients(CMMCORE_CREATE_TENSOR2D(_n,_m,vec3,0,0,0)),Mu(bpmat(n - 1)),Mv(bpmat(m - 1)),Mu_inv(std::move(Mu.inverse())),Mv_inv(std::move(Mv.inverse())){}
    Monomial2D( Tensor2D<vec3>& coeffs): n(coeffs.size()),m(coeffs[0].size()),coefficients(std::move(coeffs)),Mu(bpmat(n - 1)),Mv(bpmat(m - 1)),Mu_inv(std::move(Mu.inverse())),Mv_inv(std::move(Mv.inverse())){}
    Monomial2D(const Monomial2D& m):n(m.n),m(m.m),coefficients(m.coefficients),Mu(m.Mu),Mv(m.Mv),Mu_inv(m.Mu_inv),Mv_inv(m.Mv_inv) {}
    Monomial2D(const NURBSSurface& surf): n(surf._size[0]),m(surf._size[1]),coefficients(CMMCORE_CREATE_TENSOR2D(n,m,vec3,0,0,0)),Mu(bpmat(n - 1)),Mv(bpmat(m - 1)),Mu_inv(std::move(Mu.inverse())),Mv_inv(std::move(Mv.inverse())) {
            Tensor2D<vec3> cpts = surf.control_points3d();
            Matrix MvT = Mv.transpose();
            for (int i = 0; i < 3; ++i) {
                Matrix cfs(n, m);
                getVectorTensor2DComponent<vec3>(cpts, i, cfs);
                setVectorTensor2DComponent<vec3>(coefficients, i, Mu * cfs * MvT); // Mu @ control_points[:, :, d] @ Mv.T
            }
        }
